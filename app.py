...
# New: Root route to confirm API is running
@app.route('/')
def index():
    return jsonify({'status': 'API is running', 'message': 'Welcome to the Invoice and Client Management API!'}), 200
...
def save_invoice():
    ...
    try:
        ...
        # Check if the incoming invoice_number already exists
        if data.get('invoice_number') in [inv['invoice_number'] for inv in invoices]:
            # If it exists, generate a new unique invoice number
